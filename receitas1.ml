...
let separar linha = (   
  ...
  let rec dados (tokens, reg) = (
    match tokens with
    | [] -> reg;
    | h::t -> let (_:registo) = dados(t, reg) in preencherRegisto reg (obterValorString t) (24 - List.length t);
  )
  in dados(tokens, reg);
);;
    ...
    begin
      ...
      try let ler () = 
        ...
        let rec lerLinha lista = 
          match ler () with
          | Some linha -> lerLinha(linha::lista)
...
let imprimirRegisto registo = (
  Printf.printf "Receita: %s" registo.nome;
...
let menu = (
  let linhasLista = lerFicheiro ficheiro in
  let registosLista = criarRegistos linhasLista in
